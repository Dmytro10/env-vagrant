...
apt install -y curl inetutils-traceroute build-essential python-software-properties git cron openvpn mc mlocate
apt install -y apache2 libapache2-mod-php
apt remove -y php7-mysql

# Install PHP-5.6
apt install -y php5.6 php5.6-curl php5.6-mcrypt php5.6-cli php5.6-gd php5.6-intl php5.6-xml php5.6-xmlrpc php5.6-mysqlnd php5.6-common php5.6-mbstring php5.6-xdebug
echo " -> Update PHP conf"
sed -i 's/^post_max_size = 8M/post_max_size = 128M/' /etc/php/5.6/apache2/php.ini;
sed -i 's/^upload_max_filesize = 2M/upload_max_filesize = 128M/' /etc/php/5.6/apache2/php.ini;
...
# Install PHP-7.1
apt install -y php7.1 php7.1-curl php7.1-mcrypt php7.1-cli php7.1-gd php7.1-intl php7.1-xml php7.1-xmlrpc php7.1-mysqlnd php7.1-common php7.1-mbstring php-xdebug
echo " -> Update PHP conf"
sed -i 's/^post_max_size = 8M/post_max_size = 64M/' /etc/php/7.1/apache2/php.ini;
sed -i 's/^upload_max_filesize = 2M/upload_max_filesize = 128M/' /etc/php/7.1/apache2/php.ini;
...
cp /home/ubuntu/scripts/switch-php-to.sh /usr/bin/switch-php-to && sed -e 's/^M//g' /usr/bin/switch-php-to
chmod +x /usr/bin/switch-php-to
